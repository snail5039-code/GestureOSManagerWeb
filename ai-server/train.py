...
# ------------------------------------------------------------
# 손 정규화(학습/추론 입력 통일용)
# - 손목(0) 기준으로 이동(centering)
# - 손 크기(손목~중지 MCP(9))로 스케일 정규화(scaling)
# ------------------------------------------------------------
WRIST = 0
MIDDLE_MCP = 9


def normalize_hand_tensor(hand_t: np.ndarray) -> np.ndarray:
    """
    hand_t: (T, 2, 21, 3)
    return: same shape, but x,y normalized, z는 0으로 통일
    """
    out = hand_t.copy()
    if out.ndim != 4 or out.shape[2] != 21 or out.shape[3] < 2:
        return out
    ...
    for t in range(Tn):
        for s in range(2):
            pts = out[t, s]  # (21,3)
            if not (np.any(pts[:, 0] != 0) or np.any(pts[:, 1] != 0)):
                continue
            wrist = pts[WRIST, :2].copy()
            scale = float(np.linalg.norm(pts[MIDDLE_MCP, :2] - wrist))
            if scale < 1e-6:
                continue
            pts[:, :2] = (pts[:, :2] - wrist) / scale
            pts[:, 2] = 0.0
            out[t, s] = pts
    return out
...
class NpyDataset(Dataset):
    ...
    def __init__(self, root_dir, label_to_idx, use_face: bool = True, hand_norm: bool = True):
    ...
    def __getitem__(self, i):
        ...
        # ✅ 손 정규화(추천): 손목 기준 + 손 크기 기준
        # - 카메라 해상도/거리 차이 때문에 "특정 라벨 고정"되는 현상을 줄여줌
        if self.hand_norm:
            hand = normalize_hand_tensor(hand)
        ...
        # ✅ hand_norm을 쓰면 이미 스케일이 잡혀있으니 /1000 금지
...
def main():
    ...
    # ✅ 핵심: 손 정규화 (0이면 미사용, 1이면 사용)
    ap.add_argument("--hand_norm", type=int, default=1, help="1=손 정규화(추천), 0=미사용")
    ...
    args.hand_norm = bool(args.hand_norm)
